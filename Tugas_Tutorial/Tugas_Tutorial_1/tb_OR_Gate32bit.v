...
module tb_OR_Gate32bit();
    reg [31:0] A, B;
    wire [31:0] HasilORLogic;

    OR_Gate32bit uut(.A(A), .B(B), .HasilORLogic(HasilORLogic));
    initial begin
        $dumpfile("OR_Gate32bit.vcd");
        $dumpvars(0, tb_OR_Gate32bit);
        $monitor("A = %h, B = %h, HasilORLogic = %h,", A, B, HasilORLogic); 
