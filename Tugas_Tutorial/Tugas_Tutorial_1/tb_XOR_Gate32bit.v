...
module tb_XOR_Gate32bit();
    reg [31:0] A, B;
    wire [31:0] HasilXORLogic;

    XOR_Gate32bit uut(.A(A), .B(B), .HasilXORLogic(HasilXORLogic));
    initial begin
        $dumpfile("XOR_Gate32bit.vcd");
        $dumpvars(0, tb_XOR_Gate32bit);
        $monitor("A = %h, B = %h, HasilXORLogic = %h,", A, B, HasilXORLogic); 
